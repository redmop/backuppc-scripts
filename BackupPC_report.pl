...
# BackupPC_report
#========================================================================
# Version 0.0.4, released 2014-09-24
#========================================================================

use strict;
use warnings;              # global flag -w produces warnings from BackupPC::Lib
use lib '/usr/share/backuppc/lib';  # <<< change path prefix to match your installation
use BackupPC::Lib;
...
Cron jobs to get reports in mailbox at schedule:
 ...
 # Send report at Monday morning
 5  8  *  *  1    /somedir/BackupPC_report | mail -s "BackupPC@`hostname` `date '+\%Y-\%m-\%d \%H:\%M'` weekly report" root
 # Other days send report only if it has warnings.
 5  8  *  *  2-7  /somedir/BackupPC_report -s | mail -Es "BackupPC@`hostname` `date '+\%Y-\%m-\%d \%H:\%M'` warning" root
